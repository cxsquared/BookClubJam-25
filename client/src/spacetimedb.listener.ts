...
import { DbConnection, Decor, DeleteDecor, Inventory, MoveDecor, Package, User } from './module_bindings';
...
export class SpacetimeDBListener {
    ...
    constructor(conn: DbConnection, identity: Identity) {
        ...
        this.doorSub = conn
            ...
            .subscribe([`SELECT * FROM door WHERE current_visitor = 0x${identity.toHexString()}`]);
        ...
        conn.db.user.onUpdate((_ctx, _oldRow, newRow) => {
            ...
            console.log('user updated ' + newRow.identity.toHexString().substring(0, 8));
        ...
        conn.db.package.onInsert((_, row) => {
            ...
            if (this.currentDoorId && row.doorId == this.currentDoorId) this.packageAdded.push(row);
        });
        ...
        this.inventorySub = conn
            ...
            .subscribe(`SELECT * FROM inventory WHERE owner = 0x${identity.toHexString()}`);
        ...
        conn.db.decor.onInsert((_, row) => {
            ...
            if (this.currentDoorId && row.doorId == this.currentDoorId) this.decorAdded.push(row);
        });
