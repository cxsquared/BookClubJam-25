...
export class InventoryUi extends Container {
  ...
  constructor(onClick: (key: string, x: number, y: number) => void) {
    ...
    const bg = new Sprite(AssetManager.Assets["decor_bg"]);
    bg.y = 20;
    this.addChild(bg);
    ...
    this.decorGrid = [];
    this.decorToGrid = new Map<
      string,
      { isSmall: boolean; x?: number; y: number }
    >();
    ...
    for (const key of smallDecor) {
      const sprite = new LayoutSprite({
        texture: AssetManager.Assets[key],
        layout: true,
      });
      sprite.alpha = 0.5;
  ...
  public update(inventory: InventoryComponent) {
    const hasInventoryKeys: Set<string> = inventory.inventory.reduce<
      Set<string>
    >((acc, inventoryItem) => {
    ...
    for (const key of this.decorToGrid.keys()) {
      ...
      const sprite =
        x !== undefined ? this.decorGrid[y][x] : this.decorGrid[y][0];
